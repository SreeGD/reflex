...
Intake → Noise Check → RCA → Review → Remediation → Alert
"""
...
from backend.app.agents.nodes.review import review_node
...
def build_graph(providers: Providers, llm: Any) -> Any:
    ...
    async def _review(state: AgentState) -> AgentState:
        return await review_node(state, providers.context, llm)
    ...
    graph.add_edge("rca", "review")
    graph.add_edge("review", "remediation")
